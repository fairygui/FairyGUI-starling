	...
	public class Transition
	{
		...
		private function applyValue(item:TransitionItem):void
		{
			...
			switch (item.type)
			{
				...
				case TransitionActionType.ColorFilter:
					{
						var cf:ColorMatrixFilter = item.target.filter as ColorMatrixFilter;
						
						if (cf==null)
							cf = new ColorMatrixFilter();	
						else
							cf.reset();
						cf.adjustBrightness(item.value.f1);
						cf.adjustContrast(item.value.f2);
						cf.adjustSaturation(item.value.f3);
						cf.adjustHue(item.value.f4);
