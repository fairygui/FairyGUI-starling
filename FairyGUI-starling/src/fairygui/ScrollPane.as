...
{	
	...
	public class ScrollPane extends EventDispatcher
	{
		...
		private function handleSizeChanged(onScrolling:Boolean=false):void
		{
			...
			if (!_maskDisabled)
			{
				if(_maskContainer.mask==null)
					_maskContainer.mask = new Quad(_viewSize.x,_viewSize.y);
				else
					Quad(_maskContainer.mask).readjustSize(_viewSize.x,_viewSize.y);
			}
		...
		private function __mouseMove(evt:GTouchEvent):void
		{
			...
			var pt:Point = _owner.globalToLocal(evt.stageX, evt.stageY, ScrollPane.sHelperPoint);
