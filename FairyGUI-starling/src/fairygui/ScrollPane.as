package fairygui
{
	...
	[Event(name = "scrollEnd", type = "starling.events.Event")]
	[Event(name = "pullDownRelease", type = "starling.events.Event")]
	[Event(name = "pullUpRelease", type = "starling.events.Event")]
	public class ScrollPane extends EventDispatcher
	{
		...
		private function handleSizeChanged(onScrolling:Boolean=false):void
		{
			...
			if (!_maskDisabled)
			{
				if(_maskContainer.mask==null)
					_maskContainer.mask = new Quad(_viewWidth,_viewHeight);
				else
					Quad(_maskContainer.mask).readjustSize(_viewWidth,_viewHeight);
			}
		...
		private function __mouseDown(evt:GTouchEvent):void
		{
			...
			_xOffset = sHelperPoint.x - _container.x;
			_yOffset = sHelperPoint.y - _container.y;
		...
		private function __mouseMove(evt:GTouchEvent):void
		{
			...
			if (_scrollType == ScrollType.Vertical) 
			{
				if (!_isHoldAreaDone)
				{
					...
					{
						diff2 = Math.abs(_holdAreaPoint.x - sHelperPoint.x);
			...
			else if (_scrollType == ScrollType.Horizontal) 
			{
				if (!_isHoldAreaDone)
				{
					...
					if ((_gestureFlag & 1) != 0)
					{
						diff2 = Math.abs(_holdAreaPoint.y - sHelperPoint.y);
						if (diff < diff2)
							return;
					}
				}
		...
		private function __mouseUp(e:Event):void
		{
			...
			if (fireRelease == 1)
				dispatchEventWith(PULL_DOWN_RELEASE);
			else if (fireRelease == 2)
				dispatchEventWith(PULL_UP_RELEASE);
		}
